<script>  
  $(function () {
    ...
    var ip = 0;
  @for ($i = 30; $i > 0; $i-- )
    label_diario_pagamento_dinheiro.push(["{{today()->subDays($i)->format('d/m')}} " ])    
    dados_diario_pagamento_dinheiro.push([ {{ Manzoli2122\Salao\Atendimento\Models\Pagamento::whereDate('created_at', [  today()->subDays($i)  ])->where('formaPagamento', 'dinheiro')->sum('valor') }}   ])
    ip = ip + 1;
  @endfor
